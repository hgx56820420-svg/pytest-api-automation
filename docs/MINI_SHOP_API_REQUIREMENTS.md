...
- 所有购物车接口都需要 Bearer Token，只能读取和修改当前用户数据。
- 添加商品时商品必须存在且为 `on_sale`；同一用户重复添加时合并数量。
- 合并后的数量必须大于 0 且不超过当前库存，否则返回 `400`，数据库不得变化。
- 修改数量时路径中的 `product_id` 必须和请求体一致；不存在的购物车项返回 `404`。
- 清空和删除操作成功后返回当前购物车；删除不存在项返回 `404`。

### 13.2 优惠券规则

- 优惠券 `code` 只能包含大写字母、数字、下划线和短横线，折扣比例为 `0 < discount_percent <= 100`。
- `code` 唯一；重复创建返回 `409`，不得新增记录。
- 下单传入 `coupon_code` 时，优惠券必须为 active 且 `used_count < max_uses`，否则返回 `400`，库存、余额和订单均不变。
- 折后金额为 `round(price * quantity * (1 - discount_percent / 100), 2)`；订单保存优惠券编码和折后金额，用户余额按折后金额扣除。
- 达到使用次数后优惠券状态为 `exhausted`；取消使用该券的订单会恢复次数并重新激活。

### 13.3 库存流水规则

- 每次成功下单新增一条 `quantity_change = -quantity`、`reason = order_created` 的流水。
- 每次成功取消订单新增一条 `quantity_change = +quantity`、`reason = order_cancelled` 的流水，并关联订单 ID。
- 失败的下单、重复支付、非法取消不得产生库存流水。
- 查询接口只返回指定商品流水，并要求登录；不存在商品返回 `404`。

### 13.4 V2 验收矩阵

| 需求 ID | 场景 | HTTP 断言 | 数据断言 |
|---|---|---|---|
| REQ-CART-003 | 添加商品到购物车 | 201 | 新增或合并 cart_items |
| REQ-CART-003 | 超过库存添加 | 400 | cart_items 不变 |
| REQ-CART-001 | 跨用户查看购物车 | 200 | 不返回其他用户商品 |
| REQ-CART-005 | 删除购物车项 | 200 | cart_items 删除 |
| REQ-COUPON-002 | 创建优惠券 | 201 | coupons 新增 |
| REQ-COUPON-002 | 重复创建优惠券 | 409 | 不新增重复记录 |
| REQ-ORDER-001 | 使用优惠券下单 | 201 | 订单折后金额、余额、库存及券次数正确变化 |
| REQ-ORDER-001 | 使用耗尽优惠券下单 | 400 | 订单、余额、库存和券次数不变 |
| REQ-INVENTORY-001 | 查询库存流水 | 200 | 返回下单扣减和取消恢复流水 |
