...
## 三、当前进度（阶段 4，2026-08-07）

### 已完成并验证通过的内容

全套 **15 条测试全部通过**（`pytest tests/ -v` → `15 passed`）：
...
**`tests/test_order.py`（1 条，最有价值的一条）**：
- `test_cancel_order_restores_stock_and_balance`
...
订单模块目前只写了一条副作用回滚测试。参考 `HANDOFF.md` 第五节"服务里刻意埋好的测试点"
和 `app/routers/orders.py` 的业务逻辑，订单模块还要覆盖：
